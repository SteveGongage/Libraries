...
#chr(asc('<'))#cfquery name="UpdateUserQuery" datasource="#DSN#">
UPDATE #TableName# SET
<cfloop query="FieldNames">#Column_Name#=<cfif (is_Nullable contains "no" and len(Column_Default_Value)) and (Type_Name contains "int" or Type_Name contains "bit")>#chr(asc('<'))#cfif isDefined("form.#Column_Name#") and len(form.#Column_Name#)>#chr(asc('<'))#cfqueryparam value="##form.#Column_Name###" cfsqltype="#IIF(Type_Name contains 'int' or Type_Name contains 'bit', DE('CF_SQL_NUMERIC'), DE('CF_SQL_VARCHAR'))#" maxlength="#COLUMN_SIZE#">#chr(asc('<'))#cfelse>#REReplaceNoCase(Column_Default_Value, "[[:punct:]]", "", "all")##chr(asc('<'))#/cfif><cfelseif trim(is_Nullable)>#chr(asc('<'))#cfif isDefined("form.#Column_Name#") and len(form.#Column_Name#)>#chr(asc('<'))#cfqueryparam value="##form.#Column_Name###" cfsqltype="#IIF(Type_Name contains 'int' or Type_Name contains 'bit', DE('CF_SQL_NUMERIC'), DE('CF_SQL_VARCHAR'))#" maxlength="#COLUMN_SIZE#">#chr(asc('<'))#cfelse>NULL#chr(asc('<'))#/cfif><cfelse>#chr(asc('<'))#cfqueryparam value="##form.#Column_Name###" cfsqltype="#IIF(Type_Name contains 'int', DE('CF_SQL_NUMERIC'), DE('CF_SQL_VARCHAR'))#" maxlength="#COLUMN_SIZE#"></cfif><cfif CurrentRow NEQ Recordcount>,#chr(13)##chr(10)#</cfif></cfloop>
