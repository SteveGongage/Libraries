...
History          : December 11, 2010	: Added methods selector and filter, for jQuery style element selector and filtering.
				 ...
				 : June 18, 2012		: Limited variable scope, for variables inside methods. Also did some cleaning.
				 						: Fixed methods parents(), parent(), closest(), several function names, typos, and filter's.
				 : July 2, 2012			: Added dynamic java class laoder using Javaloader.
				 						  Updated to latest Jericho v3.2.
				 : September 20, 2012	: Fixed classpath for javaloader.
				 : December 10, 2012	: Updated to latest Jericho v3.3.
Methods:
...
<cfcomponent output="no" hint="Parser helper functions using Jericho HTML parser">
	...
	<cfset this.version.product = "Jericho Helper Parser" />
	<cfset this.version.version = "1.8" />
	<cfset this.version.author = "Pablo Schaffner" />
	<cfset this.version.releaseDate = "4/12/2010" />
	<cfset this.version.lastUpdate = "10/12/2012" /><!--- d/m/yyyy --->
    <cfset this.classdir=GetDirectoryFromPath(GetCurrentTemplatePath()) & "jericho/">
    <cfset toConsole("sparser2:classdir=#this.classdir#")>
    <!---<cfdump var="#this.classdir#"/><cfabort/>--->
    <cfset this.namespace="co_sparser2">
    <cfset this.javaloader=CreateObject("component","javaloader.JavaLoader")>
    <cfset this.initialized = false />
    <cfset this.content=""><!--- current document source content --->
    <cfset this.set_casesensitive=false><!--- be case sensitive ? --->
    <cfset this.start_time=GetTickCount()>
    <cfset this.end_time=GetTickCount()>
    <cfset this.sqlscript=ArrayNew(1)><!--- all scripts executed --->
    <cfset this.selector_tsql=ArrayNew(1)>
    <cfset this.filters=ArrayNew(1)>
    <cfset this.base_tag="">
    <cfset this.file_source="">
    <cfset this.sql_escape_attributes="value,from,where,null"><!--- attributes fields that are to be escaped (so queries work) --->
    <cfset this.lastquery=QueryNew("") />
    <!--- register here your own TagTypes --->
    <cfset this.registerTags=ArrayNew(1)><!---
    <cfset this.registerTags[1]=StructNew()>
    <cfset this.registerTags[1].type="coldfusion tag">
    <cfif server.ColdFusion.ProductName eq "BlueDragon"><cfset this.registerTags[1].tags=lcase(ArrayToList(GetSupportedtags("")))>
    <cfelse><cfset this.registerTags[1].tags="cfset,cffunction,cfloop,cfif,cfelse,cftry,cfcatch,cfcomponent,cfinvoke"></cfif>    
    <cfset this.registerTags[1].serverside=true>--->
    
    
    <cffunction name="toConsole" access="private" returntype="void" output="no">
    	<cfargument name="text" type="string" required="yes" default=""/>
		<!---<cfset var sys=createObject("java", "java.lang.System").out>
        <cfset sys.println(trim(arguments.text))>--->
    </cffunction>
    
    <!--- functions --->    
    <cffunction name="Init" access="Public" output="no" hint="Instanciates the parser.">
		...
		<cfargument name="ua" type="any" required="no" default="Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_4) AppleWebKit/536.11 (KHTML, like Gecko) Chrome/20.0.1132.57 Safari/536.11" hint="Useragent to use for requesting content.">
		<cfargument name="justInit" type="boolean" required="no" default="false" hint="Just init, dont parse DOM.">
        <cfset var source = trim(arguments.source)>
        <cfset var starttime = GetTickCount()>
        <cfset var tmpfile=""><cfset var tmpfile_path=""><cfset var tmpfile_solo="">
        <cfset var sourcecache="">
        <cfset var old_source="">
        <cfset var javaURL="">
        <cfset var reader="">
        <cfset var sourceBB="">
        <cfset toConsole("***** sparser2: Init *****")>
        <!--- cache (only for http) ? --->
        <cfset var sourcetest=lcase(left(source,5))>
        <cfset var sourcecache="">
        ...
        <!--- init ftp server object instance (2/7/2012) --->
        <cfset var cp=ArrayNew(1)><cfset var test="">
        ...
        <cfset toConsole("this.ref=eval('server.#this.namespace#')")>
        <!--- reload classpath if not loaded correctly previously --->
        <cftry>
			<cfset test=this.ref.create("net.htmlparser.jericho.HTMLElementName")>
        	<cfcatch type="any">
				<!--- redefine classpath for javaloader --->
		        <cfset cp=ArrayNew(1)>
				<!--- define ClassPath for Javaloader --->
                <cfdirectory action="list" directory="#this.classdir#" filter="*.jar" recurse="no" name="cp_fs"/>
                <cfloop query="cp_fs">
            ...
            <cftry>
                <cfsavecontent variable="sourcecache">
                    <cf_accelerate PRIMARYKEY="parser#hash(source)#a" cachedWithin="#arguments.cache#">
                        <cfhttp url="#source#" path="#tmpfile_path#" file="#tmpfile_solo#" method="get" useragent="#arguments.ua#" throwonerror="no"/>
                    </cf_accelerate>
                </cfsavecontent>
            <cfcatch type="any">
        ...
        <cfelseif arguments.ua neq "" and (sourcetest eq "http:" or sourcetest eq "https")>
        	<!--- use cfhttp (if ua is defined) --->
          	<cfhttp url="#source#" path="#tmpfile_path#" file="#tmpfile_solo#" method="get" useragent="#arguments.ua#" throwonerror="no"/>
            <cfif FileExists(tmpfile)>
            	<!--- use tmpfile as source... --->
                <cfset old_source=source>
		...
		<cfif not source contains "<" and sourcetest neq "file:" and sourcetest neq "http:">
			<cfset this.file_source=source>
    		<cfset source = "file:///" & ReplaceNoCase(source,"\","/","ALL")>
        </cfif>
        <!--- check if we need to use JavaLoader --->
        <!--- continue --->
        <cfset toConsole(":prepared to use jericho.Source:")>
        <cftry>
			<cfset javaURL=CreateObject("java","java.net.URL").init(source)>
			<cfset this.source = this.ref.create("net.htmlparser.jericho.Source").init(javaURL)>
            <cfset this.source.setLogger(JavaCast("null",""))>
            <cfcatch type="any">
            	<!--- source must be text... --->
		        ...
		        <cfset this.source = this.ref.create("net.htmlparser.jericho.Source").init(reader)>
                <cfset this.source.setLogger(JavaCast("null",""))>
            </cfcatch>
        </cftry>

        <!---<cfset this.source.fullSequentialParse()>--->
        <cfset this.content=this.source.toString()>
		<cfset this.HTMLElementName = this.ref.create("net.htmlparser.jericho.HTMLElementName")>
        <cfset this.SourceCompactor = this.ref.create("net.htmlparser.jericho.SourceCompactor")>
       
        <cfif not arguments.justInit>
			<cfset this.SourceFormatter = this.source.getSourceFormatter()>
            <cfset this.initialized = true />
            <!--- parse all tags of source --->
            <cfset this.base_tag=this.getAllTags("",true)><!--- debugtime: getting this into cache only gets it worst --->
        </cfif>
      <cfreturn this />
    </cffunction>
    ...
    <cffunction name="setSource" access="public" output="no" hint="Sets the given text as the current source content">
        ...
        <cfset var reader=""><cfset var sourceBB="">
    	<cfif this.initialized>
			<cfset reader=this.ref.create("java.io.StringReader").init(JavaCast("string",arguments.source))>
			<cfset this.source = this.ref.create("net.htmlparser.jericho.Source").init(reader)>
            <cfset this.source.setLogger(JavaCast("null",""))>
            <cfset this.source.fullSequentialParse()>
            <cfset this.content=this.source.toString()>
	        <cfset this.SourceFormatter = this.source.getSourceFormatter()>
            <cfset this.sqlscript=ArrayNew(1)>
            <cfset this.lastquery=QueryNew("")>
            <cfset this.base_tag=this.getAllTags("",true)>
            <cfreturn true/>
        <cfelse>
    ...
    <cffunction name="setCaseSensitive" access="public" output="no" hint="Use case sensitive selectors and filters.">
        ...
        <cfif arguments.sensitive>
        	<cfset this.set_casesensitive=true>
        <cfelse>
	        <cfset this.set_casesensitive=false>
        </cfif>
    </cffunction>

    <cffunction name="source" access="public" output="no" hint="Gets source object.">
    	<cfif this.initialized>
        	<cfreturn this.source>
        <cfelse>
        	<cfthrow type="sparser2.cfc" message="You must first call method Init(file)."/>
        </cfif>
    </cffunction>
    
    <cffunction name="save" access="public" output="no" hint="Saves the current content the current file or to the given file.">
	    <cfargument name="thefile" required="no" default="" type="string" hint="File where to save the current content."/>
    	<cfif isDefined("this.content")>
        	<cfif arguments.thefile neq "">
            	<cftry>
	            	<cffile action="write" file="#arguments.thefile#" output="#trim(this.content)#" nameconflict="overwrite"/>
					<cfreturn this/>
                    <cfcatch type="any">
                    	<cfthrow type="sparser2.cfc" message="Error saving file :: #cfcatch.Message#"/>
                    </cfcatch>
            	</cftry>
            <cfelse>
            	<!--- no given file output path --->
				<cfif this.file_source neq "">
                	<cffile action="write" file="#this.file_source#" output="#trim(this.content)#" nameconflict="overwrite"/>
					<cfreturn this/>
                <cfelse>
                	<cfthrow type="sparser2.cfc" message="You must first supply an output file to save the contents to, or the init source must be a valid file."/>
                </cfif>                
            </cfif>
        	<cfreturn this/>
        <cfelse>
    ...
    <cffunction name="registerTag" access="public" output="no" returntype="void" hint="Register TagType to be recognized. Slows selectors...">
        ...
        <cfset this.registerTags[arraylen(this.registerTags)+1]=StructNew()>
        <cfset this.registerTags[arraylen(this.registerTags)].type=trim(arguments.type)>
        <cfset this.registerTags[arraylen(this.registerTags)].tags=trim(arguments.tags)>
        <cfset this.registerTags[arraylen(this.registerTags)].serverside=arguments.serverside>
    </cffunction>
    ...
    <cffunction name="getTitle" access="public" output="no" hint="Get's document title.">
    	<cfset var titleElement="">
    	<cfif this.initialized>
        	<cfset titleElement=this.source.getFirstElement(this.HTMLElementName.TITLE)>
    ...
    <cffunction name="format" access="public" output="no" hint="Indents the source content.">
        ...
        <cfset var result = "">
    	<cfif this.initialized>
        	<cfset result = this.SourceFormatter.init(this.source)>
    ...
    <cffunction name="compress" access="public" output="no" hint="Compress the source content.">
    	<cfset var result="">
    	<cfif this.initialized>
        	<cfset result = this.SourceCompactor.init(this.source)>
            <cfreturn result.toString()>
        <cfelse>
        	<cfthrow type="sparser2.cfc" message="You must first call method Init(file)."/>
        </cfif>
    ...
    <cffunction name="getAllTags" access="public" output="no" hint="Gives all instances of the given tag, its attributes and content.">
        ...
        <cfset var attributesn="">
        <cfset var tmp=""><cfset var tmpnewat=""><cfset var all_attributes_names="">
		<cfset var identified=""><cfset var i_tag_type="">
        <cfset var resp="">
    	<cfif this.initialized>
            <!--- block 1: 1 ms --->
            <cfif trim(arguments.tag) neq "">
	            <cfset test=this.source.getAllElements(trim(arguments.tag))>
            <cfelse>
	            <cfset test=this.source.getAllElements()>
            </cfif>
            <!--- end block 1: 1 ms --->
                ...
                <cfloop index="Tag" array="#test#">
                    ...
                    <cftry>
                        <cfloop index="wAt" array="#tmp#">
                            ...
                            <cfif arguments.asQuery>
                            	<cfif ListContains(this.sql_escape_attributes,lcase(tmpnewat))>
            ...
            <cfloop index="Tag" array="#test#">
                ...
                <!--- tag type --->
                <cfset identified=0>
                <cfloop index="wTagType" from="1" to="#arraylen(this.registerTags)#" step="+1">
                	<cfif ListFindNoCase(this.registerTags[wTagType].tags,result[arraylen(result)].i_tag_name) gt 0>
						<cfset identified=wTagType>
                    </cfif>
                </cfloop>
                <!--- assign --->
                <cfif identified neq 0>
					<cfset result[arraylen(result)].i_tag_is_serverside=this.registerTags[identified].serverside>
                    <cfset result[arraylen(result)].i_tag_type=this.registerTags[identified].type>
                <cfelse>
                	<cftry>
                        ...
                        <cfcatch type="any">
                        	<cfset result[arraylen(result)].i_tag_type="">
                            <cfset result[arraylen(result)].i_tag_is_serverside=false>
                        </cfcatch>
                    </cftry>
                </cfif>
                ...
                <cfset result[arraylen(result)].i_begin_row=this.source.getRowColumnVector(Tag.getBegin()).getRow()>
                <cfset result[arraylen(result)].i_begin_col=this.source.getRowColumnVector(Tag.getBegin()).getColumn()>--->
                <cfset result[arraylen(result)].i_end=Tag.getEnd()><!---
                <cfset result[arraylen(result)].i_end_row=this.source.getRowColumnVector(Tag.getEnd()).getRow()>
                <cfset result[arraylen(result)].i_end_col=this.source.getRowColumnVector(Tag.getEnd()).getColumn()>
                ...
                <cfset attributesn=Tag.getAttributes()>
                <cftry>
                    <cfloop index="wAttr" array="#attributesn#">
                    	<cfif not arguments.asQuery>
	...
	<!--- EXTRACTION METHOD 2: selector --->
 	<cffunction name="selector" access="public" output="no" hint="Helper function to access tags and elements as jQuery selector usage.">
        ...
        <cfset var base=this.base_tag><!--- base query --->
        <cfset var nill="">
        <cfset var selection=""><cfset var multiple="">
        <cfset var wSel=""><cfset var parent="">
        <cfset var parent_where_clause=""><cfset var pt_where=""><cfset var ptw_where="">
        <cfset var where=""><cfset var tmp="">
        <cfset var parent_cfcmd="">
        <cfset var tmp_begin=""><cfset var tmp_end=""><cfset var wRange="">
        <cfset var newo="">
        <cfset var resp="">
        <cfset clean_vars()><!--- clean previously used variables.. --->
        <!---<cfset this.lastquery="">--->
        ...
        <cfelseif base.recordCount neq 0>
            ...
            <cfloop index="cSel" from="1" to="#listlen(selection,' ')#" step="+1">
                ...
                <cfset parent_where_clause=this.sqlscript[arraylen(this.sqlscript)].sql>
                ...
                <cfelseif cSel neq 1 and parent.recordCount gt 0>
                    ...
                    <cfset this.sqlscript[arraylen(this.sqlscript)+1]=sql>
					<cfif wSel contains this.filters[arraylen(this.filters)] and trim(this.filters[arraylen(this.filters)]) neq "">
                        <cfset newo=i_filter(this.filters[arraylen(this.filters)])><!--- apply current selector filter to all previous queries --->
                    </cfif>
 	...
 	<cffunction name="selectorHelperMain" access="private" output="no" hint="Helper function to access tags and elements as jQuery selector usage.">
        ...
        <cfset var base="">
        <cfset var resp_a=""><cfset var resp_b=""><cfset var resp="">
        <cfset var resp_ab=""><cfset var resp_c="">
        <cfset this.sqlscript=ArrayNew(1)><!--- reset sql script --->
        <cfif IsQuery(arguments.basequery)>
        	<cfset base=arguments.basequery>
        <cfelse>
			<cfset base=this.base_tag><!--- base query --->
		</cfif>
        <!--- continue --->
    	...
    	<cfif isQuery(this.lastquery)>
        	<cfif trim(arguments.parameter) neq "">
            	<cftry>
                    ...
                    <cfset this.sqlscript[arraylen(this.sqlscript)+1]=sq_script>
    ...
    <cffunction name="parents" access="public" output="no" hint="Get the ancestors of each element in the current set of matched elements, optionally filtered by a selector.">
        ...
        <cfargument name="clean" required="no" type="boolean" default="true" hint="Clean unused columns."/>
		<cfset var i_begins=""><cfset var i_ends="">
        <cfset var prenew="">
        <cfset var not_repeat="">
        <cfset var c_begin=""><cfset var c_end=""><cfset var c_sql="">
        <cfset var newonep=""><cfset var newone="">
        <cfset var sq_script="">
    	<cfif isQuery(this.lastquery)>
        	<!--- get i_begin and i_end for each records in lastquery --->
            <!--- captura position de inicio y fin de ultima busqueda --->
            <cfset i_begins=""><cfset i_ends="">
            <cfloop query="this.lastquery">
            	<cfif ListFindNoCase(i_begins,this.lastquery.i_begin) eq 0>
                	<cfset i_begins=ListAppend(i_begins,this.lastquery.i_begin)>
                	<cfset i_ends=ListAppend(i_ends,this.lastquery.i_end)>
                </cfif>
            </cfloop>
            <cfif arguments.selector eq ""><cfset arguments.selector="*"></cfif>
            <!--- filter with given selector --->
            <cfset selectorHelper(this.base_tag,arguments.selector,true)><!--- busca el selector en la base TOTAL  --->
            <cfset prenew=this.lastquery>
            <!--- get ALL the ancestors for each record in the previous selector --->
            <cfset not_repeat="">
			<cfloop index="testParent" from="1" to="#listlen(i_begins)#" step="+1">
            	<cfset c_begin=ListGetAt(i_begins,testParent)>
                <cfset c_end=ListGetAt(i_ends,testParent)>
                <cfset c_sql="select i_row from prenew where i_begin < #c_begin# and i_tag_type not like '%declaration'"><!--- and i_end > #c_end#  --->
            ...
            <cfreturn this/>
        <cfelse>
        	<cfthrow type="parser2::parents" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction> 
    ...
    <cffunction name="parent" access="public" output="no" hint="Get the parent of each element in the current set of matched elements, optionally filtered by a selector.">
        ...
        <cfargument name="clean" required="no" type="boolean" default="true" hint="Clean unused columns."/>
        <cfset var i_begins=""><cfset var i_ends="">
        <cfset var prenew=""><cfset var not_repeat="">
        <cfset var c_begin=""><cfset var c_end=""><cfset var c_sql="">
        <cfset var newonep=""><cfset var sq_script="">
        <cfset var newone="">
    	<cfif isQuery(this.lastquery)>
            ...
            <cfif arguments.selector eq ""><cfset arguments.selector="*"></cfif>
            <!--- filter with given selector --->
            <cfset selectorHelper(this.base_tag,arguments.selector,true)>
            <cfset prenew=this.lastquery>
            <!--- get ALL the ancestors for each record in the previous selector --->
            <cfset not_repeat="">
			<cfloop index="testParent" from="1" to="#listlen(i_begins)#" step="+1">
            	<cfset c_begin=ListGetAt(i_begins,testParent)>
                <cfset c_end=ListGetAt(i_ends,testParent)>
                <cfset c_sql="select * from prenew where i_begin < #c_begin# and i_tag_type not like '%declaration'"><!--- and i_end > #c_end#  (because parent maybe the 'parent' for this tag itself 18/6/2012) ---->
                <cfquery name="newonep" dbtype="query" result="sq_script">
                #PreserveSingleQuotes(c_sql)#
                </cfquery>
            ...
            <cfreturn this/>
        <cfelse>
        	<cfthrow type="parser2::parent" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction> 
    ...
    <cffunction name="closest" access="public" output="no" hint="Get the first ancestor element that matches the selector, beginning at the current element and progressing up through the DOM tree.">
        ...
        <cfargument name="clean" required="no" type="boolean" default="true" hint="Clean unused columns."/>
        <cfset var i_begins=""><cfset var i_ends="">
        <cfset var prenew=""><cfset var last_record="">
        <cfset var c_sql=""><cfset var newonep=""><cfset var newone="">
        <cfset var sq_script="">
    	...
    	<cfif isQuery(this.lastquery) and arguments.selector neq "">
            ...
            <cfif this.lastquery.recordCount neq 0>
            	<cfset i_begins=this.lastquery.i_begin>
                <cfset i_ends=this.lastquery.i_end>
				<!--- filter with given selector --->
                <cfset selectorHelper(this.base_tag,arguments.selector,true)>
                <cfset prenew=this.lastquery>
                <!--- get ALL the ancestors for each record in the previous selector --->
                ...
                <cfset last_record="">
                <cfset c_sql="select * from prenew where i_begin < #i_begins# and i_tag_type not like '%declaration'"><!--- i_begins  and i_end > #i_ends# 18/6/2012 --->
                <cfquery name="newonep" dbtype="query" result="sq_script">
                #PreserveSingleQuotes(c_sql)#
                </cfquery>
		...
		<cfelseif arguments.selector eq "">
        	<cfthrow type="parser2::closest" message="You must specify the selector attribute for the closest method."/>
        <cfelse>
        	<cfthrow type="parser2::closest" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction> 
    ...
    <cffunction name="filter" access="public" output="no" hint="Filters the lastquery query as with the complete given selector.">
        ...
        <cfset var prenew=""><cfset var not_repeat="">
        <cfset var c_begin=""><cfset var c_end=""><cfset var c_sql="">
        <cfset var newonep=""><cfset var sq_script=""><cfset var newone="">
        <cfif isQuery(this.lastquery) and trim(arguments.selector) neq "">
			...
			<!--- end --->
		<cfelseif arguments.selector eq "">
            <cfthrow type="parser2::filter" message="You must specify the selector attribute for the filter method."/>
        <cfelse>
            <cfthrow type="parser2::filter" message="You must first call method getAllTags or selector."/>
        </cfif>
        <cfreturn this/>
    </cffunction>    
	...
	<cffunction name="html" access="public" output="no" hint="Get or sets the HTML contents of the first element in the set of matched elements.">
        ...
        <cfset var currenti=""><cfset var new_content=""><cfset var plus_len_content="">
        <cfif isQuery(this.lastquery)>
        	<cfif this.lastquery.recordCount neq 0>
                ...
                <cfelse>
                    ...
                    <cfset new_content=this.content>
                    ...
                    <cfif len(this.content) neq len(new_content)>
        ...
        <cfelse>
	        <cfthrow type="parser2::html" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction>
    ...
    <cffunction name="removeTags" access="public" output="no" hint="Removes all tags from the given string - helper function">
        ...
        <cfset var tmpres=trim(REReplaceNoCase(arguments.string,"<[^>]*>","","ALL"))>
		<cfreturn tmpres/>
    </cffunction>
    ...
    <cffunction name="getNumbers" access="public" returntype="array" output="no" hint="Gets an array with all numbers (as text) found on the current selector.">
    	<cfset var theText=text()>
        <cfset var tmpNew=ArrayNew(1)>
	...
	<cffunction name="text" access="public" output="no" hint="Get the combined text contents of each element in the set of matched elements, including their descendants.">
        ...
        <cfset var i_new_content="">
        <cfset var i_new_contentb="">
        <cfset var currenti=""><cfset var new_content=""><cfset var plus_len_content=0>
        <cfif isQuery(this.lastquery)>
        	<cfif this.lastquery.recordCount neq 0>
				<cfif arguments.content eq "">
                    ...
                    <cfset i_new_content="">
                    <cfloop query="this.lastquery">
                    	<cfset i_new_content=i_new_content & this.lastquery.i_content>
                    </cfloop>
                    <!--- removetags from i_new_content --->
                    <cfset i_new_contentb=trim(REReplaceNoCase(i_new_content,"<[^>]*>","","ALL"))>
                    <!--- return i_new_content --->
                    <cfreturn i_new_contentb/>
                <cfelse>
                    <!--- order from begining position.. --->
                    <cfset currenti=this.lastquery>
                    <cfset new_content=this.content>
        ...
        <cfelse>
	        <cfthrow type="parser2::text" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction>
	...
	<cffunction name="replaceWith" access="public" output="no" hint="Replaces each instances of the matched elements with the given content.">
        ...
        <cfset var currenti="">
        <cfset var new_content="">
        <cfset var plus_len_content=0>
        <cfif isQuery(this.lastquery)>
        	<cfif this.lastquery.recordCount neq 0>
                <!--- order from begining position.. --->
                <cfset currenti=this.lastquery>
        ...
        <cfelse>
	        <cfthrow type="parser2::replaceWith" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction>
    ...
    <cffunction name="removeEmptyColumns" access="private" returntype="query" output="no" hint="Cleans the given query to delete columns without values in all rows">
        ...
        <cfset var in_use="">
        <cfset var used=false>
        <cfset var resp=""><cfset var sq_script="">
    ...
    <cffunction name="i_filter" access="public" output="no" hint="Filters the lastquery query as with jQuery filters.">
	    <cfargument name="parameter" required="yes" type="string" default="" hint="Filter."/>
        <cfset var param=trim(arguments.parameter)><cfset var base=""><cfset var resp=""><cfset var sq_script=""><cfset var even_rows="">
        <cfset var odd_rows=""><cfset var queryall="">
        <cfset var tsql_current=""><cfset var t_where=""><cfset var tw_where="">
        <cfset var i_rows=""><cfset var tmp_begin=""><cfset var tmp_end=""><cfset var tmp_where="">
		<cfset var tmpnum=""><cfset var tmp_sub_param="">
        <cfset var queryall=""><cfset var tmp_test=""><cfset var tmpsql=""><cfset var tmpsel="">        
    	...
    	<cfif isQuery(this.lastquery)>
            ...
            <cfswitch expression="#param#">
            	...
            	<cfcase value=":first-child,:last-child,:only-child">
                	<cfset queryall=this.base_tag>
                    <!--- extract current tsql script where clause --->
                    <cfset tsql_current=this.sqlscript[arraylen(this.sqlscript)].sql>
                    ...
                    <cfif arraylen(this.selector_tsql) gt 1>
	                    <cfset base=this.selector_tsql[arraylen(this.selector_tsql)-1]>
                    <cfelse>
                    ...
                    <cfloop index="wfc" from="1" to="#base.recordcount#" step="+1">
                        ...
                        <cfset tmp_test="select i_row from queryall where #tmp_where# and i_tag_name='#this.selector_tsql[arraylen(this.selector_tsql)].i_tag_name#'">
                        ...
                        <cfif test.recordCount neq 0>
                        	<cfif param eq ":first-child">
								<cfif ListFindNoCase(i_rows,test.i_row) eq 0>
                                    <cfset i_rows=ListAppend(i_rows,test.i_row)>
                                </cfif>
                            ...
                            <cfelse>
								<cfif ListFindNoCase(i_rows,test.i_row[test.recordCount]) eq 0>
                                    <cfset i_rows=ListAppend(i_rows,test.i_row[test.recordCount])>
                                </cfif>
                            </cfif>
                        </cfif>
                    </cfloop>
                    <cfif listlen(i_rows) gt 0>
                    	<cfquery name="resp" dbtype="query" result="sq_script">
                        select * from queryall where i_row in (#i_rows#)
                        </cfquery>
                        <cfset this.sqlscript[arraylen(this.sqlscript)+1]=sq_script>
                    <cfelse>
                    	<cfset resp=QueryNew(base.columnList)>
                ...
                <cfcase value=":checkbox">
                	<cfif ListFindNoCase(base.columnList,"type") neq 0>
                        <cfquery name="resp" dbtype="query" result="sq_script">
                ...
                <cfdefaultcase>
                	...
                	<cfif left(param,len(":nth-child(")) eq ":nth-child(" and right(param,1) eq ")">
                        ...
                        <cfloop index="wfc" from="1" to="#base.recordcount#" step="+1">
                            ...
                            <cfset tmp_test="select i_row from queryall where #tmp_where# and i_tag_name='#this.selector_tsql[arraylen(this.selector_tsql)].i_tag_name#'"><!--- i_tag_name needed here, because parent doesnt where tagname --->
                            <cfquery name="test" dbtype="query" result="sq_script">
                            #PreserveSingleQuotes(tmp_test)#
                            </cfquery>
                            <cfset this.sqlscript[arraylen(this.sqlscript)+1]=sq_script>
                            <cfif test.recordCount neq 0>
                                ...
                                <cfelseif IsNumeric(tmp_sub_param)>
                                	<cfif test.recordCount gte tmp_sub_param><!--- only add row if nth position exists --->
										<cfif ListFindNoCase(i_rows,test.i_row[tmp_sub_param]) eq 0>
                                            <cfset i_rows=ListAppend(i_rows,test.i_row[tmp_sub_param])>
                                        </cfif>
                                    </cfif>
                                </cfif>
                            </cfif>
                    ...
                    <cfelseif left(param,4) eq ":lt(" and right(param,1) eq ")"><!--- :lt(x) --->
                        ...
                        <cfelse>
                            <cfquery name="resp" dbtype="query" result="sq_script">
                            select * from base where #base.i_row[tmpnum]#>i_row
                            </cfquery>
                        </cfif>
                        <cfset this.sqlscript[arraylen(this.sqlscript)+1]=sq_script>
                        <cfset this.lastquery=resp>
        ...
        <cfelse>
        	<cfthrow type="sparser2::i_filter" message="You must first call method getAllTags or selector."/>
        </cfif>
    </cffunction>
	...
	<!--- called from selectorHelperMain --->
 	<cffunction name="selectorHelper" access="private" output="no" hint="Helper">
        ...
        <cfset var base=arguments.query>
        <cfset var selecto=""><cfset var filtro="">
        <cfset var selecto_a=""><cfset var atribo="">
        <cfset var tmp=""><cfset var resp=""><cfset var sq_script="">
        <cfset var tmp_tag=""><cfset var tmp_class=""><cfset var tmp_id="">
        <cfset var where_clause=""><cfset var req_columns=""><cfset var condition="">
        <cfset var tmp_name=""><cfset var tmp_value=""><cfset tmp_operator="">
        <cfset var new_where=""><cfset var all_req_attributes_exist=false>
        <cfset var base_col=""><cfset var tmp_q=""><cfset var respb="">
        <!--- --->
        <cfif listlen(arguments.parameter,":") gt 1>
        	<cfset selecto=ListFirst(arguments.parameter,":")>
            <cfset filtro=ListLast(arguments.parameter,":")><!--- i_filter(x) --->
        <cfelse>
        ...
        <cfif atribo neq "" and resp.recordCount neq 0>
            ...
            <cfloop index="wAtribo" list="#atribo#" delimiters=",">
                ...
                <cfif listlen(condition,"=") gt 1><!--- [param=value] --->
                    ...
                    <cfloop index="wSelAtE" list="#this.sql_escape_attributes#">
					...
					<cfif this.set_casesensitive eq false>
        ...
        <cfset this.selector_tsql[arraylen(this.selector_tsql)+1]=resp><!--- this is needed here, because i_filter(x) uses it inside! --->
        <cfset this.lastquery=resp><!--- this is needed here, because i_filter(x) uses it inside! --->
        <!--- apply filter lastquery --->
		<cfif filtro neq "" and arguments.applyfilter>
        	<cfset resp=i_filter(filtro)>
			<cfset this.lastquery=resp><!--- this is needed here, because after filter, the result must be available for other selectors! --->
            <cfset this.selector_tsql[arraylen(this.selector_tsql)+1]=resp><!--- this is needed here, because after filter must be available for other selectors! --->
        </cfif>
        <cfset this.filters[arraylen(this.filters)+1]=filtro>
        <cfreturn resp/>
    </cffunction>
    <!--- --->
    ...
    <cffunction name="findSelector" returntype="struct" output="no" hint="returns array with suggested css selector's for finding the given value." access="public">
		...
		<cfset var toreturn=StructNew()><cfset var el_selector=""><cfset var preferred_tagtype=""><cfset var the_type=""><cfset var position=""><cfset var break="">
		<cfset var value_col=""><cfset var specificb=""><cfset var tst=""><cfset var chosen_rows=""><cfset var lv_value=""><cfset var last_lv="">
        <cfset var lv_row=""><cfset var cols=""><cfset var the_row=""><cfset var value_col="">
        <cfset var break=false><cfset var chosen_position=""><cfset var biggest_row=""><cfset var biggest_size="">
        <cfset var test_wi_tags=""><cfset var smallest_row=""><cfset var smallest_size="">
        <cfset var lv_smallest=""><cfset var actual=""><cfset var lv_row=""><cfset var the_tag="">
        <cfset var pos_end=""><cfset var pos_begin=""><cfset var pos_irow=""><cfset var previous_sql="">
        <cfset var el_selector=""><cfset var tmp=""><cfset var test=""><cfset var toreturn=StructNew()>
        <cfset var specific="">
        <!--- --->
        <!---<cfif not isDefined("internal")>
            ...
            <cfset internal=StructNew()>
            <cfset this.base_tag=parser.selector("*").query()>
        </cfif>--->
        ...
        <cfswitch expression="#the_type#">
            <cfcase value="exact,exacto,igual,identico">
                ...
                select * from this.base_tag where i_content='#arguments.fvalue#'
                </cfquery>
            </cfcase>
            <cfcase value="partial,parcial,contenido">
                ...
                <cfset tst="select * from this.base_tag where i_content not like '%<%' and i_content not like '%>%' and i_tag_name not like 'script'">
                <cfquery name="specificb" dbtype="query">
                ...
                <cfquery name="specificb" dbtype="query">
                select * from this.base_tag where i_row in (#chosen_rows#)
                </cfquery>
            </cfcase>
            ...
            <cfcase value="similar,parecido">
                ...
                <cfquery name="specificb" dbtype="query">
                select * from this.base_tag where i_row=#lv_row#
                </cfquery>
            </cfcase>
            <cfcase value="attribute,attributes,atributo,atributos">
                <!--- exact attribute --->
                <!--- search using loop --->
                <cfset cols=this.base_tag.columnList>
                <cfloop index="qr" from="1" to="#this.base_tag.recordCount#" step="+1">
                    <!--- for each column, search the attribute value --->
                    <cfloop index="qcol" list="#cols#">
                        <!--- exact match --->
                        <cfif Evaluate("this.base_tag.#qcol#[qr]") eq arguments.fvalue>
                            <!--- found --->
                            <cfset the_row=Evaluate("this.base_tag.i_row[qr]")>
                            <cfquery name="specificb" dbtype="query">
                            select * from this.base_tag where i_row=#the_row#
                            </cfquery>
            ...
            <cfcase value="attributep,attributesp,atributop,atributosp,contenido de atributo,contenido en atributo,contenido atributo">
                ...
                <cfloop index="qr" from="1" to="#this.base_tag.recordCount#" step="+1">
                    ...
                    <cfloop index="qcol" list="#cols#">
                        <!--- partial match --->
                        <cfif Evaluate("this.base_tag.#qcol#[qr]") contains arguments.fvalue>
                            <!--- found --->
                            <cfset the_row=Evaluate("this.base_tag.i_row[qr]")>
                            <cfquery name="specificb" dbtype="query">
                            select * from this.base_tag where i_row=#the_row#
        ...
        <cfif specific.recordCount neq 0>
            ...
            <!--- search parent tags --->
            <cfset previous_sql="select * from this.base_tag where i_end>#pos_end# and i_begin<#pos_begin#">
